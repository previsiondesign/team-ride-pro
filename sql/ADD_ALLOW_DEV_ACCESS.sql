-- Add allow_dev_access column to user_roles table
-- Controls whether a user can access Developer Mode
ALTER TABLE user_roles ADD COLUMN IF NOT EXISTS allow_dev_access BOOLEAN DEFAULT FALSE;

-- Must drop first because the return type is changing (new column added)
DROP FUNCTION IF EXISTS public.get_users_with_auth();

-- Recreate with allow_dev_access in the return type
CREATE FUNCTION public.get_users_with_auth()
RETURNS TABLE (
    user_id UUID,
    role TEXT,
    email TEXT,
...
AS $$
    SELECT
        ...
        COALESCE(ur.allow_dev_access, FALSE) AS allow_dev_access
